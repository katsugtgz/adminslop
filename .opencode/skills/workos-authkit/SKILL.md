---
name: workos-authkit
description: Implement, review, debug, and maintain WorkOS AuthKit authentication, organizations, memberships, roles, permissions, sessions, and related identity architecture using official WorkOS documentation and the SDK installed in this project.
...
# WorkOS AuthKit Skill (OpenCode, project-scoped)

> **Source.** This skill is the official WorkOS coding-agent skill
> (`workos@0.6.1`, installed by `npx workos@latest skills install`) adapted for
> OpenCode. It is mirrored at `.opencode/skills/workos-authkit/` so future
> sessions in this repository discover it natively. The reference files under
> `references/` are the unmodified official WorkOS references.

## Project-specific directives (apply to EVERY response in this repo)

This repository is **EduAdmin Pro Premium** — a multi-tenant Indonesian
school-administration platform. See `CONTEXT.md` (domain glossary) and
`docs/architecture/identity-and-access.md` (identity architecture) before
> writing WorkOS integration code. The fixed stack is **Next.js 15.3 (App
> Router) + WorkOS AuthKit + Supabase (DB/RLS only) + Drizzle + Tailwind v4**.

### Tenancy & authorization mapping (NON-NEGOTIABLE)

These map the project domain language onto WorkOS resources. Honor them exactly:

| Project term (CONTEXT.md) | WorkOS resource | Notes |
|---|---|---|
| **Satuan Pendidikan** (school/institution) | `Organization` | The primary operational data boundary = tenant. |
| **Keanggotaan Satuan Pendidikan** | `OrganizationMembership` | M:N — a `Pengguna` may belong to many Satuan Pendidikan. |
| **Pengguna** (person) | `User` | One identity across all memberships. |
| `tenant_role` | WorkOS `Role` / membership `roleSlug` | **Never superuser.** Enforced server-side. |
| **Instansi Pengelola** (oversight org) | (deferred) | Optional org-of-orgs layer — do NOT provision yet. |

### Hard rules for this project

Before producing any code, answer, or suggestion involving WorkOS in this repo:

1. **Inspect the installed WorkOS SDK version first.** Run
   `npm ls @workos-inc/authkit-nextjs @workos-inc/node` and use only APIs
   documented for that version. This project pins `@workos-inc/authkit-nextjs@^4`.
2. **Read the relevant official docs before guessing.** Use the local copy at
   `docs/vendor/workos/` or the `workos-docs` OpenCode reference. Do NOT invent
   WorkOS API shapes, CLI commands, or dashboard click-paths.
3. **Prefer the official SDK over handwritten OAuth.** AuthKit already owns the
   callback route (`src/app/api/auth/callback/route.ts`), middleware
   (`src/middleware.ts`), and session sealing. Do not reimplement.
4. **Authentication ≠ authorization.** A signed-in user is not authorized to
   act. Every protected server route/server action must perform an explicit
   authorization check against `tenant_role` + the active Satuan Pendidikan.
5. **Enforce tenant isolation server-side.** Organization-scoped data must be
   filtered by the active `Organization` id. Never trust a client-supplied org id
   without confirming it matches an authenticated membership. Do authorization
   in the server component / route / middleware — never only in client UI.
6. **Preserve existing middleware & proxy behavior.** `src/middleware.ts` uses
   `authkitMiddleware()`. When extending matchers, never remove the auth matcher
   or `//` route.
7. **Secrets are server-only.**
   - Never expose `WORKOS_API_KEY`.
   - Never expose `WORKOS_COOKIE_PASSWORD`.
   - Never put server credentials in `NEXT_PUBLIC_*`, browser bundles, logs, API
     responses, or source control. `WORKOS_CLIENT_ID` is the only value safe to
     expose client-side.
   - Never store WorkOS access tokens, refresh tokens, raw sessions, or
     passwords in application database tables — AuthKit owns session state in the
     httpOnly cookie. The app DB stores only stable identifiers where needed.
8. **Use stable identifiers, not emails.** Key app data to WorkOS `user.id`,
   `organization.id`, `membership.id` (immutable) rather than email/display name.
9. **Sandbox for development.** Local dev uses the WorkOS **sandbox**
   environment (`sk_test_*`). Do not touch the production environment without
   explicit owner approval.
10. **Defer enterprise features.** Do NOT enable SSO, Directory Sync, SCIM,
    MFA enforcement, Custom Domains, Admin Portal embeds, FGA, Vault, Radar, or
    other paid/enterprise functionality unless a concrete requirement exists in
    `hyperplan/`/`CONTEXT.md` AND the owner explicitly approves. Call out any
    paid or enterprise-only feature *before* implementing it.
11. **Run lint + typecheck + build after auth changes**
    (`npm run lint`, `npx tsc --noEmit`, `npm run build`).

### When to defer to the official router below

The directives above are project policy. For WorkOS-feature mechanics
(AuthKit flow, RBAC API, webhook shapes, migration steps, CLI usage), follow the
official **WorkOS Skill Router** that follows — it maps requests to the
`references/*.md` files. Those references are the authoritative mechanics; this
section is the authoritative policy.
